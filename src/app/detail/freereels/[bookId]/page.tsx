...
export default function FreeReelsDetailPage() {
  ...
  // if the API provided explicit episode array, we can show it; otherwise fallback to count
  const episodeList = drama.episode_list || [];
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
            ...
            <div className="space-y-6">
              ...
              {(episodeList.length > 0 || drama.episode_count) && (
                <div className="mt-6">
                  ...
                  <div className="grid grid-cols-5 gap-2">
                    {episodeList.length > 0
                      ? episodeList.map((ep: any, idx: number) => (
                          <Link
                            key={ep.id || idx}
                            href={`/watch/freereels/${bookId}?ep=${idx + 1}`}
                      ...
                      : Array.from({ length: drama.episode_count || 0 }).map((_, idx) => (
                          <Link
                            key={idx}
                            href={`/watch/freereels/${bookId}?ep=${idx + 1}`}
                            className="aspect-square flex items-center justify-center rounded-lg text-sm font-medium transition-all bg-white/5 text-white/70 hover:bg-white/10 hover:text-white"
