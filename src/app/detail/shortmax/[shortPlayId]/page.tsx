...
import { useShortMaxDetail, useShortMaxAllEpisodes } from "@/hooks/useShortMax";
...
export default function ShortMaxDetailPage() {
  ...
  const { data: allData, isLoading: epLoading } = useShortMaxAllEpisodes(shortPlayId || "");
  const episodes = allData?.episodes || [];

  if (isLoading || epLoading) {
        ...
        <div className="relative max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8">
            ...
            <div className="space-y-6">
              ...
              {episodes.length > 0 && (
                <div className="mt-6">
                  <h3 className="font-semibold text-foreground mb-2">Daftar Episode</h3>
                  <div className="grid grid-cols-5 gap-2">
                    {episodes.map((ep) => (
                      <Link
                        key={ep.episodeNumber}
                        href={`/watch/shortmax/${data.shortPlayId}?ep=${ep.episodeNumber}`}
                        className="aspect-square flex items-center justify-center rounded-lg text-sm font-medium transition-all bg-white/5 text-white/70 hover:bg-white/10 hover:text-white"
                      >
                        {ep.episodeNumber}
                      </Link>
                    ))}
